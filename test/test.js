...
describe("Gatoswap Test", function () {
  it("Gatoswap Deploy & ERC20 Test", async function() {
    ...
    const gatoswapRouter = await GatoswapRouter.deploy(gatoswapFactory.address, wETH.address);

    const GatoToken = await ethers.getContractFactory("ERC20");
    GatoToken.deploy()
